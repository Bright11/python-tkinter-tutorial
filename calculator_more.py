from tkinter import *

root = Tk()
root.geometry("400x500")
root.title("Calculator")

decimal_number = 0.0  # Initial decimal number

myframe = Frame(root, width=100, height=200, bg="blue", relief=SUNKEN)
myframe.pack(side=TOP)

e = Entry(myframe, width=20, borderwidth=5, bg="gray", fg="white", font=("Arial", 20))
e.grid(row=0, column=0, columnspan=3)

def button_click(number):
    current = e.get() 
    e.delete(0, END)
    e.insert(0, str(current) + str(number))

def button_clear():
    e.delete(0, END)
    
def button_add():
    first_number = e.get()
    global f_num
    global math
    math = "addition"
    f_num = float(first_number)  # Convert to float
    e.delete(0, END)
    
def update_button_text():
    global button_point, decimal_number
    button_point.config(text="{:.1f}".format(decimal_number))

def button_equal():
    second_number = e.get()
    e.delete(0, END)
    if math == "addition":
        e.insert(0, f_num + float(second_number))  # Convert to float
    if math == "subtraction":
        e.insert(0, f_num - float(second_number))  # Convert to float
    if math == "multiplication":
        e.insert(0, f_num * float(second_number))  # Convert to float
    if math == "division":
        e.insert(0, f_num / float(second_number))  # Convert to float

def button_subtract():
    first_number = e.get()
    global f_num
    global math
    math = "subtraction"
    f_num = float(first_number)  # Convert to float
    e.delete(0, END)

def button_multiply():
    first_number = e.get()
    global f_num
    global math
    math = "multiplication"
    f_num = float(first_number)  # Convert to float
    e.delete(0, END)

def button_divide():
    first_number = e.get()
    global f_num
    global math
    math = "division"
    f_num = float(first_number)  # Convert to float
    e.delete(0, END)

button_1 = Button(myframe, text="1", padx=40, pady=20, command=lambda: button_click(1))
button_2 = Button(myframe, text="2", padx=40, pady=20, command=lambda: button_click(2))
button_3 = Button(myframe, text="3", padx=40, pady=20, command=lambda: button_click(3))
button_4 = Button(myframe, text="4", padx=40, pady=20, command=lambda: button_click(4))
button_5 = Button(myframe, text="5", padx=40, pady=20, command=lambda: button_click(5))
button_6 = Button(myframe, text="6", padx=40, pady=20, command=lambda: button_click(6))
button_7 = Button(myframe, text="7", padx=40, pady=20, command=lambda: button_click(7))
button_8 = Button(myframe, text="8", padx=40, pady=20, command=lambda: button_click(8))
button_9 = Button(myframe, text="9", padx=40, pady=20, command=lambda: button_click(9))
button_0 = Button(myframe, text="0", padx=40, pady=20, command=lambda: button_click(0))
button_point = Button(myframe, text="{:.1f}".format(decimal_number), padx=40, pady=20, command=lambda: button_click("."))
button_add = Button(myframe, text="+", padx=39, pady=20, command=button_add)
button_equal = Button(myframe, text="=", padx=91, pady=20, command=button_equal)
button_clear = Button(myframe, text="C", padx=40, pady=20, background="red", fg="white", command=button_clear)
button_subtract = Button(myframe, text="-", padx=40, pady=20, command=button_subtract)
button_multiply = Button(myframe, text="*", padx=40, pady=20, bg="#FF9F0A", command=button_multiply)
button_divide = Button(myframe, text="/", padx=40, pady=20, command=button_divide)

button_1.grid(row=3, column=0)
button_2.grid(row=3, column=1)
button_3.grid(row=3, column=2)
button_4.grid(row=2, column=0)
button_5.grid(row=2, column=1)
button_6.grid(row=2, column=2)
button_7.grid(row=1, column=0)
button_8.grid(row=1, column=1)
button_9.grid(row=1, column=2)
button_0.grid(row=4, column=0)
button_clear.grid(row=4, column=2)
button_point.grid(row=4, column=1)
button_add.grid(row=5, column=0)
button_equal.grid(row=5, column=1, columnspan=2)
button_subtract.grid(row=6, column=0)
button_multiply.grid(row=6, column=1)
button_divide.grid(row=6, column=2)

update_button_text()

root.mainloop()
